...
export const onRequest = async (context) => {
  try {
    ...
    if (url.pathname === "/status") {
      if (!env.STATUS_KV) return new Response(JSON.stringify({ error: "KV Binding STATUS_KV is missing" }), { status: 500, headers: corsHeaders });
    ...
    // Pass through to next handler (static assets)
    return next();
  } catch (error) {
